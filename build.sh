...
# Builds a debug or release APK/AAB for Android using Capacitor.
# Requires a local Android SDK (devcontainer has this).
...
Examples:
  ./build.sh
  ./build.sh --apk --debug
  ./build.sh --apk --release --keystore ~/my.jks --ks-pass secret --key-pass secret
  ./build.sh --aab --release --keystore ~/my.jks --ks-pass secret --key-pass secret
EOF
}
...
if [[ "$OUTPUT" == "aab" && "$VARIANT" == "debug" ]]; then
...
[[ -d "$APP_DIR" ]] || die "app/ directory not found."
